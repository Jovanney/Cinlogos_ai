...
import { CurrentFormRef } from "./multi-step-form";
    ...
    if (formRef.current)
      if (me !== currentForm && me !== goToForm)
        ...
        formRef.current.querySelectorAll("form *").forEach((e) => {
          if (e.classList.contains("tabable")) e.setAttribute("tabindex", "0");
        ...
        if (formRefValue && ref) ref.current = formRefValue;
      }
...
function mapForms(
  ...
  goToForm: number | undefined
) {
  const mappedForms = forms.map(({ form }, index) => {
    if (goToForm) {
      const me = index + 1;
      ...
      if (direction >= 1) {
        if (me >= currentNumber - direction && me <= currentNumber) {
          ...
          return (
            <FromWrapper
              key={"form-component-" + me}
              form={form}
              me={me}
              posOffset={-120 * offset}
      ...
      else if (me === leftPrev || me === previous || me === rightPrev) {
...
const StepFormsContainer = ({
  ...
  forms: multiModalType;
  currentNumber: number;
  previousHook: [number, (x: number) => void];
  goToForm: number | undefined;
  setGoToForm: (x: number | undefined) => void;
  gtfTemp: boolean;
  setGtfTemp: (x: boolean) => void;
}) => {
  const [previous, setPrevious] = previousHook;
  const formsRendered = useMemo(
    () => mapForms(forms, previous, currentNumber, goToForm),
  ...
  return (
    <div
      ...
      style={
        currentNumber !== previous || (goToForm !== undefined && !gtfTemp)
          ? currentNumber > previous
            ? { left: -120 * formDiffNumber + "%" }
            : { left: 120 * formDiffNumber + "%" }
          : { left: 0 }
      }
      onTransitionEnd={() =>
        gtfTemp
          ? disableGoToForm()
          : currentNumber !== previous && setPrevious(currentNumber)
      }
    >
      {formsRendered}
    </div>
